...
const PlanID = () => {
  ...
  const dailyTradesNeeded = useAppSelector((state) =>
    selectTradesPerDay(state, plan?.id, parseFloat(dailyPNLNeeded))
  );
  const PNLPerTradeNeeded = (
    parseFloat(dailyPNLNeeded) / dailyTradesNeeded
  ).toFixed(2);
      ...
      {plan && (
        <>
          ...
          <div className="flex flex-col max-w-[250px] gap-2">
            ...
            <span>
              Trade volume: {((plan.deposit * plan.risk) / 100).toFixed(2)}$
            </span>
          </div>
        </>
      )}
